...
namespace hs {
	...
	perm_node* empty_node() {
			perm_node *t = (perm_node *) malloc(sizeof(perm_node));
			t->l = NULL;
			t->r = NULL;
			t->p = NULL;
			t->height = 0;
	...
	perm_tree *build_perm_tree(perm *pi) {
			perm_tree *t = (perm_tree *) malloc(sizeof(perm_tree));
			t->leafs = (perm_node **) malloc( length_perm(pi) * sizeof(perm_node*));
			t->roots[0] = build_perm_tree_(t, pi, 0, length_perm(pi));
			t->size = length_perm(pi);
			t->pos = (int *) malloc( length_perm(pi) * sizeof(int));
			t->pos_version = (long int *) calloc(length_perm(pi), sizeof(long int));
			t->current_version = 1;
			return t;
	}
