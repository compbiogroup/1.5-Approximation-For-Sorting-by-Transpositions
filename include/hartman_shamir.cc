...
namespace hs {
	...
	int transposition_shattered(cicle_graph *g, perm_tree *t, three_cicle cicle1, three_cicle cicle2, three_cicle cicle3) {
			...
			if(i == -1) error((char *) "Five cicle is not oriented.");
	...
	int remove_2_cicles(cicle_graph *g) {
			...
			bool *two_cicles = (bool *) calloc(size, sizeof(bool));
			int *es = (int *) malloc(size * sizeof(int));
	...
	int hartman_shamir(int *vet1, int *vet2, int size) {
			int dist=0;

			perm *pi = build_and_rename_perm(vet1,vet2,size,Trans);
